...
<script>
	function fn_notice_detail(idx){
		$("#notice_idx").val(idx);
		$("#data_form").attr("action", "/career/notice/noticeDetail.do").submit();
	}
									...
									<c:when test="${fn:length(noticeList) > 0}">
										<c:forEach items="${noticeList}" var="row" varStatus="status">
											<tr onclick="fn_notice_detail(${row.NOTICE_IDX})" style="cursor:pointer">
												...
												<td class="td_subject">${row.NOTICE_TITLE}</td>
												<td class="td_nick sv_use">${row.NOTICE_WRITER}</td>
												<td class="td_date">${row.NOTICE_INSERT_DATE}</td>
												<td class="td_num">${row.NOTICE_CLICK_COUNT }</td>
											</tr>
										</c:forEach>
									</c:when>				
									<c:otherwise>
